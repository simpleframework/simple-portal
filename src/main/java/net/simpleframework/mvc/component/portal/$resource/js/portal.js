...
function _lo_initPortal(l, actName) {
  ...
  function updatePortalColumns(a) {
    ...
    tip.next(".portal").select(">.column").each(function(c) {
      ...
      ulIDs.push(ulID);
      var liIDs = [];
      c.select(">ul>li").each(function(li) {
        liIDs.push(li.id);
      });
      p[ulID] = liIDs.join("#");
    });
...
function __portal_add_module(module) {
  ...
  $Actions['addLayoutModule'](pagelet ? pagelet.params + "&module=" + module
      : "module=" + module);
}
...
function _win_columns_change(initValue) {
